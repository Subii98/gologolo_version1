export default class AppsterModel {
    ...
    logoTextInputDelete(){
        this.view.createScreenClose();
    }

    logoEditTextInput(){
        //this.view.editTextInput(textFill);
        this.view.createEditTextScreen();
    }

    logoEditTextInputDelete(){
        this.view.createEditTextScreenClose();
    }
    ...
    goList(){
        let c = this.buildAppWork(this.recentWork);
        
        //this.prependWork(c);
    }

    confirmNewLogo(){
        let logoname = this.view.testinput();
        let newLogo = this.buildAppWork(this.recentWork);
        //let check = true;
        for (let i = 0; i < this.recentWork.length; i++) {
            let testWork = this.recentWork[i];
            if (testWork.getName() === logoname){  
                this.view.confirmModalOpen();
                this.view.createScreenClose();
                return;
            }
        }
        if(logoname.length <1){
            ...
            newLogo.setName(logoname);
            this.prependWork(newLogo);
            this.view.createScreenClose();
    }


    // updateText(){
    //     let newText = this.view.testinput();
    //     this.setText(newText);
    // }
    closeConfirm(){
        this.view.confirmModalClose();
        this.view.createScreen();
    }
    trash(){
